### This script plots measures:
# Genetic progress measures: Inbred and Hybrid Variances & Selection Acc.
# Heterosis-related measures: Additive, Dominance portion, Base/Inbred/Panmictic Heterosis 
...
nReps = 30
scenarios = c("PS","PedigreeBlup_Seed5", 
              "Seed_GSconst_pheno_noECT5",
              "Seed_GSunconst_pheno_noECT5",
              "ECT_GS5")
title = "25% parents replaced" # plot title
...
rawData$scenario = sub("^PS5$","PS",rawData$scenario)
rawData$scenario = sub("^PedigreeBlup_Seed5$","Seed-Ped",rawData$scenario)
# rawData$scenario = sub("^PedigreeBlup$","Pedigree",rawData$scenario)
rawData$scenario = sub("^Seed_GSconst_pheno_noECT5$","Seed-GSc",rawData$scenario)
rawData$scenario = sub("^Seed_GSunconst_pheno_noECT5$","Seed-GSunc",rawData$scenario)
rawData$scenario = sub("^ECT_GS5$","ECT-GS",rawData$scenario)
...
a.25 <- ggplot(temp[temp$year >= 0,], aes(x = year,y = mean,color=scenario)) +
  geom_line(size = 1.3) + 
...
ggsave(filename = paste("PlotProgress_finalPlots/Gain_Seed.png"), plot = a.25, width = 5, height = 5, scale = 1.1)
...
a2.25 <- ggplot(temp[temp$year >= 0,], aes(x = year,y = mean,color=scenario)) +
  geom_line(size = 1.3) + 
  geom_line(data = temp[temp$year >= 0 & temp$scenario == "PS",], aes(x = year, y = mean), size = 1.3) +
  # scale_colour_manual(values=colorPalette) +
  scale_colour_manual(values=c("black",wes_palette("FantasticFox1")[2:5])) +
  ...
  theme(panel.background = element_blank(),
        ...
        legend.position = c(0.2,0.8))
        # legend.position = c(0.28,0.76)) 
ggsave(filename = paste("PlotProgress_finalPlots/Gain_ECT.png"), plot = a2.25, width = 5, height = 5, scale = 1.1)
...
b.25 <- ggplot(temp[temp$year >= 0,], aes(x = year,y = mean,color=scenario)) +
  geom_line(size = 1.3) + 
  geom_line(data = temp[temp$year >= 0 & temp$scenario == "PS",], aes(x = year, y = mean), size = 1.3) +
  # scale_colour_manual(values=colorPalette) +
  scale_colour_manual(values=c("black",wes_palette("FantasticFox1")[2:5])) +
...
ggsave(filename = paste("PlotProgress_finalPlots/Variance.png"), plot = b.25, width = 5, height = 5, scale = 1.1)

b1.25 <- ggplot(rawData[rawData$year == 40,], aes(x=scenario, y=varSeed,color=scenario)) +
  geom_boxplot() +
...
ggsave(filename = paste("PlotProgress_finalPlots/Variance_boxplot.png"), plot = b1.25, width = 6, height = 5, scale = 1.1)
...
c.25 <- ggplot(temp[temp$year >= 0,], aes(x = year,y = mean,color=scenario)) +
  geom_line(size = 1.3) + 
  geom_line(data = temp[temp$year >= 0 & temp$scenario == "PS",], aes(x = year, y = mean), size = 1.3) +
  # scale_colour_manual(values=colorPalette) +
  scale_colour_manual(values=c("black",wes_palette("FantasticFox1")[2:5])) +
  ...
  theme(panel.background = element_blank(),
        ...
        # legend.position = c(0.28,0.76)) 
ggsave(filename = paste("PlotProgress_finalPlots/Accuracy_Seed.png"), plot = c.25, width = 5, height = 5, scale = 1.1)
...
c1.25 <- ggplot(temp[temp$year >= 0,], aes(x = year,y = mean,color=scenario)) +
  geom_line(size = 1.3) + 
  geom_line(data = temp[temp$year >= 0 & temp$scenario == "PS",], aes(x = year, y = mean), size = 1.3) +
  # scale_colour_manual(values=colorPalette) +
  scale_colour_manual(values=c("black",wes_palette("FantasticFox1")[2:5])) +
  ...
  theme(panel.background = element_blank(),
        ...
        legend.position = c(0.8,0.8))
        # legend.position = c(0.28,0.76)) 
ggsave(filename = paste("PlotProgress_finalPlots/Accuracy_ACT.png"), plot = c1.25, width = 5, height = 5, scale = 1.1)

summary25 <- ggarrange(a.25,b.25,c.25, nrow = 1, ncol=3, common.legend = TRUE, legend="bottom")
print(summary25 <- annotate_figure(summary25, top = text_grob(title, color = "black", face = "bold", size = 14)))
ggsave(filename = paste("PlotProgress_finalPlots/Summary.png"), plot = summary25, width = 12, height = 4, scale = 1.1)
save.image("PlotProgress_finalPlots/Data25.RData")
