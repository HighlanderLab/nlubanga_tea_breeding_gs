...
scenarioName = "Seed_GSunconst_pheno_noECT5"
...
for(year in (nBurnIn + 1):nCycles){ 
  ...
  Parents = c(Parents[c(6:20)],
              selectInd(ParentPool, nInd = 5, use = "ebv"))
  ...
  output$accSeed[year] = cor(gv(ACT1), ebv(ACT1)) # accuracy based on 2000 inds
  ACT1 = selectInd(ACT1, nInd = ncloneSeedGSuncontACT, use = "ebv")
  ...
  # Year 1
  # Crossing block
  F1 = randCross(Parents, nCrosses=ncross, nProgeny = nProgSeed_GSunconst)
